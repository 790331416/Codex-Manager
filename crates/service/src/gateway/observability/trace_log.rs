...
pub(crate) struct ResponseFailureContextLog<'a> {
    pub trace_id: &'a str,
    pub path: &'a str,
    pub adapter: &'a str,
    ...
    pub last_attempt_error: Option<&'a str>,
    pub final_error: Option<&'a str>,
    pub stream_terminal_seen: bool,
    pub stream_terminal_error: Option<&'a str>,
    pub delivery_error: Option<&'a str>,
    pub upstream_error_hint: Option<&'a str>,
...
pub(crate) fn log_response_failure_context(params: ResponseFailureContextLog<'_>) {
    let ResponseFailureContextLog {
        trace_id,
        path,
        adapter,
        account_id,
    ...
    mark_trace_has_error(trace_id);
    let line = format!(
        "ts={} event=RESPONSE_FAILURE_CONTEXT trace_id={} path={} adapter={} candidate={}/{} account_id={} model={} upstream_url={} upstream_status={} final_status={} stage={} failover={} has_more_candidates={} last_attempt_error={} final_error={} terminal_seen={} terminal_error={} delivery_error={} upstream_hint={} upstream_request_id={} upstream_cf_ray={} upstream_auth_error={} upstream_identity_error_code={} upstream_content_type={} last_sse_event={}",
        current_trace_ts(),
        sanitize_text(trace_id),
        sanitize_text(path),
        sanitize_text(adapter),
        ...
        upstream_status_code,
        final_status_code,
        sanitize_text(failure_stage),
        if will_failover { "true" } else { "false" },
        if has_more_candidates { "true" } else { "false" },
        sanitize_text(last_attempt_error.unwrap_or("-")),
        sanitize_text(final_error.unwrap_or("-")),
        if stream_terminal_seen { "true" } else { "false" },
        sanitize_text(stream_terminal_error.unwrap_or("-")),
        sanitize_text(delivery_error.unwrap_or("-")),
        sanitize_text(upstream_error_hint.unwrap_or("-")),
